export function TableContainer({ children, className = '', ...props }) {
  return <div className={`overflow-x-auto ${className}`} {...props}>{children}</div>;
}

export function Table({ children, className = '', ...props }) {
  return <table className={`min-w-full divide-y divide-gray-200 text-sm ${className}`} {...props}>{children}</table>;
}
