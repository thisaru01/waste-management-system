...
export default function AdminSensorSim() {
  ...
  const onSubmitRow = async (id) => {
    const d = draft[id] || {};
    const payload = {};
    if (d.fillLevelPercent !== undefined && d.fillLevelPercent !== "")
      payload.fillLevelPercent = Number(d.fillLevelPercent);
    if (d.weightKg !== undefined && d.weightKg !== "")
      payload.weightKg = Number(d.weightKg);
    ...
    try {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("binUpdated", { detail: updated })
        );
      } catch {
      ...
      try {
        ...
        if (st === "collected" || st === "collector") {
          const record = {
            ...
            location:
              (updated.location &&
                (updated.location.description || updated.location)) ||
              "—",
            type: updated.type || "—",
            fill:
              (updated.fillLevelPercent ??
                updated.fillNumeric ??
                updated.fill ??
                updated.fillLevel ??
                "–") + " %",
          ...
          try {
            window.dispatchEvent(
              new CustomEvent("binCollected", { detail: updated })
            );
          } catch {
            // ignore
          }
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <Card>
        <CardHeader
          ...
          subtitle="View bins and simulate sensor readings (fill level and weight)."
        />
        <CardContent>
                  ...
                  <tr key={b._id} className="hover:bg-gray-50">
                    ...
                    <TD>
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-gray-600">
                          {b.weightKg?.toFixed?.(2) ?? b.weightKg ?? 0}
                        </span>
                        <Input
                          type="number"
                          min={0}
                          ...
                          placeholder="kg"
                          onChange={(e) =>
                            onDraftChange(b._id, "weightKg", e.target.value)
                          }
                          className="w-28"
                        />
                      </div>
                    ...
                    <TD>
                      <select
                        ...
                        onChange={(e) =>
                          onDraftChange(b._id, "status", e.target.value)
                        }
                      >
                        <option value="normal">Normal</option>
                        <option value="needs-collection">
                          Needs collection
                        </option>
                        <option value="collected">Collected</option>
                        <option value="unauthorized-collection">
