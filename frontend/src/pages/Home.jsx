import { Link } from 'react-router-dom';
import { useAuth } from '../context/AuthContext.jsx';
import WasteTypeChart from '../components/ui/WasteTypeChart.jsx';
import WasteFillChart from '../components/ui/WasteFillChart.jsx';

export default function Home() {
  const { user } = useAuth();
  
  
  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-semibold text-gray-900">Welcome</h1>
      {user ? (
        <>
          <div className="rounded-lg border border-gray-200 bg-white p-4">
            <div className="text-sm text-gray-700">Signed in as <span className="font-medium">{user.email}</span></div>
            <div className="text-sm text-gray-700">Roles: {user.roles.map((r) => r.displayName).join(', ')}</div>
            <div className="mt-3 text-sm text-gray-600">Use the nav bar above to access admin tools.</div>
            {user.roles.some((r) => r.name === 'admin') && (
              <div className="mt-3">
                <Link to="/admin" className="inline-flex items-center justify-center rounded-md bg-blue-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-blue-700">Go to Admin</Link>
              </div>
            )}
          </div>
          <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-lg border border-gray-200 bg-white p-4">
              <div className="text-sm text-gray-500">Total Waste Collected</div>
              <div className="mt-1 text-2xl font-semibold text-gray-900">8,500 kg</div>
            </div>
            <div className="rounded-lg border border-gray-200 bg-white p-4">
              <div className="text-sm text-gray-500">Missed Collections</div>
              <div className="mt-1 text-2xl font-semibold text-gray-900">5</div>
            </div>
            <div className="rounded-lg border border-gray-200 bg-white p-4">
              <div className="text-sm text-gray-500">Active Zones</div>
              <div className="mt-1 text-2xl font-semibold text-gray-900">12</div>
            </div>
            <div className="rounded-lg border border-gray-200 bg-white p-4">
              <div className="text-sm text-gray-500">Efficiency %</div>
              <div className="mt-1 text-2xl font-semibold text-gray-900">85%</div>
            </div>
          </div>

          <div className="mt-6">
            <h2 className="text-lg font-semibold text-gray-900">Deep Dive Analysis</h2>
            <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
              <WasteTypeChart />
              <WasteFillChart />
            </div>
          </div>

          
        </>
      ) : (
        <p className="text-gray-700">Please log in</p>
      )}
    </div>
  );
}
