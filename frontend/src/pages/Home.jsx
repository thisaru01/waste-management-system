    ...
    (async () => {
      try {
        ...
        for (const b of transformed) {
          const isOverflow =
            (b.fillNumeric ?? 0) >= 100 ||
            (b.status && String(b.status).toLowerCase() === "overflow");
          if (!isOverflow) continue;
          const loc =
            (b.location && (b.location.description || b.location)) || "Unknown";
        ...
        console.debug(
          "failed to fetch flagged bins for dashboard alert",
          e?.message || e
        );
      }

      // Fetch unauthorized-collection bins for authorities
      try {
        const unauth = await listUnauthorizedBins();
        if (!mounted) return;
        // group unauthorized bins by location
        const map = new Map();
        for (const b of unauth || []) {
          const loc =
            (b.location && (b.location.description || b.location)) || "Unknown";
          map.set(loc, (map.get(loc) || 0) + 1);
        }
        setUnauthAlerts(Array.from(map.entries()));
      } catch (e) {
        // don't surface errors on home; this is a best-effort alert
        console.debug(
          "failed to fetch unauthorized bins for dashboard alert",
          e?.message || e
        );
      }
    })();
    ...
    <div className="space-y-4">
      ...
      {user ? (
        <div className="rounded-lg border border-gray-200 bg-white p-4">
          ...
          {/* Unauthorized collection alerts (authority only) */}
