...
import { Table, TableContainer, TBody, TH, TD, THead } from '../components/ui/Table.jsx';
...
export default function CollectorSchedule() {
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader title="My Schedule" subtitle={`Assigned bins for ${fullName || 'collector'}`} />
        <CardContent>
          {loading && <p className="text-sm text-gray-600">Loading assigned bins…</p>}
              ...
              <Table>
                ...
                <TBody>
                  {bins.length === 0 ? (
                    <tr>
                      <TD colSpan={9}>
                        <p className="text-sm text-gray-600">No bins assigned to you yet.</p>
                      </TD>
                    </tr>
                  ) : (
                    bins.map((b) => (
                      <tr key={b._id} className="hover:bg-gray-50">
                        ...
                        <TD>{b?.owner ? `${b.owner.firstName || ''} ${b.owner.lastName || ''}`.trim() : '-'}</TD>
                        ...
                        <TD>{b.assignedAt ? new Date(b.assignedAt).toLocaleString() : '-'}</TD>
                      </tr>
                    ))
                  )}
                </TBody>
